...
$pidentity= strip_tags($_POST['pidentity']);


$res= $db->prepare("SELECT count(*) as totalcount FROM audio_calls where id=:id and identity=:identity order by id");
$res->execute(array(':id' =>$pid, ':identity' =>$pidentity ));
$t_row = $res->fetch();
$totalcount = $t_row['totalcount'];

if($totalcount == 0){
//echo "<div style='background:red;color:white;padding:10px;border:none;'>Data has been Uploaded Yet.. <b></b></div>";
echo 11;
exit();
}

$result = $db->prepare("SELECT * FROM audio_calls where id=:id and identity=:identity order by id DESC limit :row1, :rowpage");
$result->bindValue(':rowpage', (int) trim($page_row_call), PDO::PARAM_INT);
$result->bindValue(':row1', (int) trim($queryid), PDO::PARAM_INT);
$result->bindValue(':id', trim($pid), PDO::PARAM_STR);
$result->bindValue(':identity', trim($pidentity), PDO::PARAM_STR);
